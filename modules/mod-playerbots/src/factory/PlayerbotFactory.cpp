...
static bool IsSpellFitBySkills(uint32 spellId, uint8 race, uint8 cClass)
{
    ...
    {
        if (GetSkillRaceClassInfo(sla->second->SkillLine, race, cClass))
            return true;
    }
...
void PlayerbotFactory::InitAvailableSpells()
{
    ...
    for (uint32 trainerId : trainerIdCache[bot->getClass()])
    {
             ...
             itr != trainer_spells->spellList.end(); ++itr)
        {
            ...
            if (tSpell->learnedSpell[0] && !IsSpellFitBySkills(tSpell->learnedSpell[0], bot->getRace(), bot->getClass()))
                continue;
            ...
            for (uint8 j = 0; j < 3; ++j)
            {
                ...
                if (!bot->IsSpellFitByClassAndRace(tSpell->learnedSpell[j]))
                {
                    learn = false;
                    break;
                }

                if (!IsSpellFitBySkills(tSpell->learnedSpell[j], bot->getRace(), bot->getClass()))
                {
                    learn = false;
                    break;
                }
            ...
            {
                // Validate the spell being cast as well
                SpellInfo const* castSpellInfo = sSpellMgr->GetSpellInfo(tSpell->spell);
                if (castSpellInfo)
                {
                    bool canCast = true;
                    for (uint8 k = 0; k < MAX_SPELL_EFFECTS; ++k)
                    {
                        if (castSpellInfo->Effects[k].Effect == SPELL_EFFECT_LEARN_SPELL)
                        {
                            uint32 learnedSpell = castSpellInfo->Effects[k].TriggerSpell;
                            if (learnedSpell && !IsSpellFitBySkills(learnedSpell, bot->getRace(), bot->getClass()))
                            {
                                canCast = false;
                                break;
                            }
...
void PlayerbotFactory::InitSpecialSpells()
{
         ...
         i != sPlayerbotAIConfig->randomBotSpellIds.end(); ++i)
    {
        ...
        if (!bot->IsSpellFitByClassAndRace(spellId))
            continue;
        if (!IsSpellFitBySkills(spellId, bot->getRace(), bot->getClass()))
            continue;
